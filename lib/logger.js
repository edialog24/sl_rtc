...
var Logger = /** @class */ (function () {
    ...
    Logger.prototype.printToConsole = function (level, messages) {
        var prefix = sprintf_js_1.sprintf('%s %s: ', exports.levels[level], this.module);
        var args = [prefix];
    ...
    Logger.prototype.log = function (level, args) {
        ...
        var time = new Date();
        var lineStart = sprintf_js_1.sprintf('%1s %4u %3s %u %02u:%02u:%02u.%0-3u %-30s', exports.levels[level], time.getUTCFullYear(), months[time.getUTCMonth()], time.getUTCDate(), time.getUTCHours(), time.getUTCMinutes(), time.getUTCSeconds(), time.getUTCMilliseconds(), this.module);
        var compoundMessage = '';
        var i = 0;
        for (; i < args.length; i++) {
